<template>
    <v-card outlined>
        <v-card-title>
            Edms # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <Boolean label="저장유무" v-model="item.저장유무" :editMode="editMode" @change="change" />
            </div>
        </v-card-text>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/edms/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
