<template>
    <div style="margin: 0 -15px 0 -15px;">
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>ApplicationFile</v-list-item-title>
                    </v-list-item-content>
                </template>
                ...
                <v-list-item v-for="(item, idx) in value" :key="idx">
                    <v-list-item-content>
                        ...
                        <v-list-item-subtitle>
                            파일Id :  {{item.파일Id }} <br>
                            파일명 :  {{item.파일명 }} <br>
                            파일확장자명 :  {{item.파일확장자명 }} <br>
                            파일유형코드 :  {{item.파일유형코드 }} <br>
                            파일순서 :  {{item.파일순서 }} <br>
                            사용여부 :  {{item.사용여부 }} <br>
                            등록일시 :  {{item.등록일시 }} <br>
                            수정일시 :  {{item.수정일시 }} <br>
        ...
        <ApplicationFile v-if="editMode && tick" offline :isNew="true" :inList="true"
...
<script>
    import ApplicationFile from '../ApplicationFile.vue';

    export default {
        name: 'ApplicationFileManager',
        components: {
            ApplicationFile
        },
        props: {
