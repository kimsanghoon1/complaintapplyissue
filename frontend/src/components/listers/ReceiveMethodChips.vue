<template>
    <div style="margin: 0 -15px 0 -15px;">
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>ReceiveMethod</v-list-item-title>
                    </v-list-item-content>
                </template>
                ...
                <v-list-item v-for="(item, idx) in value" :key="idx">
                    <v-list-item-content>
                        ...
                        <v-list-item-subtitle>
                            수령방법코드 :  {{item.수령방법코드 }} <br>
                            수령방법명 :  {{item.수령방법명 }} <br>
                            수령방법구분명 :  {{item.수령방법구분명 }} <br>
                            등록자Id :  {{item.등록자Id }} <br>
                            등록일시 :  {{item.등록일시 }} <br>
                            수령자Id :  {{item.수령자Id }} <br>
                            수령일시 :  {{item.수령일시 }} <br>
        ...
        <ReceiveMethod v-if="editMode && tick" offline :isNew="true" :inList="true"
...
<script>
    import ReceiveMethod from '../ReceiveMethod.vue';

    export default {
        name: 'ReceiveMethodManager',
        components: {
            ReceiveMethod
        },
        props: {
